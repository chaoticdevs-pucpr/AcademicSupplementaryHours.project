...
if(isset($_POST['versao'], $_POST['data_manual'], $_POST['pontos_objetivo'])){
    $versao = $_POST['versao'];
    $data_manual = $_POST['data_manual'];
    $pontos_objetivo = (int)$_POST['pontos_objetivo'];

    $stmt = $conexao->prepare("INSERT INTO MANUAL_HC(curso_id, pontos_objetivo, versao, data) VALUES(?,?,?,?)");
    $stmt->bind_param("iiss", $curso_id, $pontos_objetivo, $versao, $data_manual);
    $stmt->execute();
