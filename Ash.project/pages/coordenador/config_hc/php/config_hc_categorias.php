...
if(isset($_GET['categoria_id'])){
    ...
    $stmt = $conexao->prepare("SELECT c.id, c.nome, c.max_pontos AS max_horas, c.max_pontos, c.descricao, m.versao FROM CATEGORIA c INNER JOIN MANUAL_HC m ON m.id = c.manual_hc_id WHERE c.id = ? AND m.curso_id = ? LIMIT 1");
...
if(isset($_GET['versao'])){
    $versao = $_GET['versao'];
    $stmt = $conexao->prepare("SELECT c.id, c.nome, c.max_pontos AS max_horas, c.max_pontos, c.descricao FROM CATEGORIA c INNER JOIN MANUAL_HC m ON m.id = c.manual_hc_id WHERE m.curso_id = ? AND m.versao = ? ORDER BY c.nome");
