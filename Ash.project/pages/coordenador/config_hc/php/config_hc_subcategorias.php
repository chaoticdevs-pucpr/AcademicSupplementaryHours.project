...
if(isset($_GET['subcategoria_id'])){
    ...
    $stmt = $conexao->prepare("SELECT s.id, s.nome, s.quant_pontos, s.descricao, c.id AS categoria_id FROM SUBCATEGORIA s INNER JOIN CATEGORIA c ON c.id = s.categoria_id INNER JOIN MANUAL_HC m ON m.id = c.manual_hc_id WHERE s.id = ? AND m.curso_id = ? LIMIT 1");
...
$stmt = $conexao->prepare("SELECT s.id, s.nome, s.quant_pontos, s.descricao FROM SUBCATEGORIA s INNER JOIN CATEGORIA c ON c.id = s.categoria_id INNER JOIN MANUAL_HC m ON m.id = c.manual_hc_id WHERE m.curso_id = ? AND s.categoria_id = ? ORDER BY s.nome");
