...
if(isset($_GET['id'])){
    ...
    $stmt = $conexao->prepare("DELETE s FROM SUBCATEGORIA s INNER JOIN CATEGORIA c ON c.id = s.categoria_id INNER JOIN MANUAL_HC m ON m.id = c.manual_hc_id WHERE c.id = ? AND m.curso_id = ?");
    $stmt->bind_param("ii", $categoria_id, $curso_id);
    $stmt->execute();
    $stmt->close();

    $stmt = $conexao->prepare("DELETE c FROM CATEGORIA c INNER JOIN MANUAL_HC m ON m.id = c.manual_hc_id WHERE c.id = ? AND m.curso_id = ?");
    $stmt->bind_param("ii", $categoria_id, $curso_id);
    $stmt->execute();
    ...
    if($affected > 0){
        $conexao->commit();
        $retorno = ['status' => 'ok', 'mensagem' => 'Categoria excluída com sucesso.', 'data' => []];
    } else {
        $conexao->rollback();
        $retorno = ['status' => 'nok', 'mensagem' => 'Nao foi possivel excluir a categoria.', 'data' => []];
    }
} else {
