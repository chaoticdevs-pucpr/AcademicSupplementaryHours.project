        ...
        <table class="w-full text-left border-collapse min-w-[860px] text-sm">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="px-4 py-3 sm:px-6 sm:py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Matrícula</th>
    ...
    if (!tabela || tabela.length === 0) {
        ...
        for(var i = 0; i < tabela.length; i++){
            ...
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm font-medium text-slate-400">${est.id}</td>
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm font-bold text-purple-600 font-mono">${est.matricula_id || '---'}</td>
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm font-semibold text-slate-900 break-words">${est.nome}</td>
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm text-slate-600 break-words">${est.email}</td>
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm text-slate-600 font-mono">${est.cpf || '---'}</td>
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm text-slate-600">${est.celular || est.telefone || '---'}</td>
