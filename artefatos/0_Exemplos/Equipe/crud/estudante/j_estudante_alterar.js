...
async function carregarDados() {
    try {
        ...
        const resTurmas = await fetch("../z_php/carregar_dados.php?tipo=turmas");
...
async function alterar(){
    ...
    if (!nome || !cpf || !email || !endereco || !celular || !curso_id || !turma_id) {
    ...
    const retorno = await fetch("p_estudante_alterar.php?id="+id,  {
        method: "POST",
        body: fd
    });
    ...
    if(resposta.status == "ok"){
        ...
        window.location.href = 'h_estudante_index.html';
    }else{
...
async function buscarDados(id){
    const retorno = await fetch("p_estudante_get.php?id="+id);
    const resposta = await retorno.json();
    if(resposta.status == "ok" && resposta.data.length > 0){
        ...
        document.getElementById("turma_id").value = dados.turma_id;
    }else{
