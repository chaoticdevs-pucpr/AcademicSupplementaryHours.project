<?php
    ...
    if (!isset($_POST['nome'], $_POST['cpf'], $_POST['email'], $_POST['endereco'], $_POST['celular'], $_POST['curso_id'])) {
        $retorno['status'] = 'nok';
    ...
    $stmt = $conexao->prepare("INSERT INTO coordenador(nome, cpf, email, endereco, curso_id) VALUES (?, ?, ?, ?, ?)");
    ...
    $stmt->bind_param("ssssi", $nome, $cpf, $email, $endereco, $curso_id);
    ...
    if($stmt->affected_rows > 0){
        $coordenador_id = $conexao->insert_id;
        
        $stmt_telefone = $conexao->prepare("INSERT INTO telefone_coo(coordenador_id, celular, residencial) VALUES (?, ?, ?)");
        $stmt_telefone->bind_param("iss", $coordenador_id, $celular, $residencial);
