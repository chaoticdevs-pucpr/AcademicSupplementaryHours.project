<?php
    ...
    if(isset($_GET['id'])) {
        ...
        $stmt = $conexao->prepare("UPDATE coordenador SET nome = ?, cpf = ?, email = ?, endereco = ?, curso_id = ? WHERE id = ?");
        $stmt->bind_param("ssssii", $nome, $cpf, $email, $endereco, $curso_id, $id);
        ...
        if (!$stmt->execute()) {
            ...
            if (strpos($erro, 'cpf') !== false || strpos($erro, 'Duplicate entry') !== false) {
                ...
                $retorno['mensagem'] = 'Erro ao alterar: ' . $erro;
            }
        ...
        if($stmt->affected_rows >= 0){
            $stmt_telefone = $conexao->prepare("UPDATE telefone_coo SET celular = ?, residencial = ? WHERE coordenador_id = ?");
